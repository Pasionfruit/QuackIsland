/**
 * Polyland relay server.
 *
 * Deliberately dumb: it hands out room codes and forwards payloads between the
 * people in a room. It never simulates a game, so every future Polyland game
 * can use it as-is - the host's browser is always the authority.
 *
 *   npm run server
 */
...
log(`Polyland relay listening on ws://0.0.0.0:${PORT}`)
log('Friends on your network join at http://<your-ip>:5173')
