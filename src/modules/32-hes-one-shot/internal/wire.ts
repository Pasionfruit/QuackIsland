 ...
 *
 * The host sends the clock, every player - where they are, which way they look,
 * when they were eliminated and by whom, how many they have eliminated, whether
 * they have left - and the last half second of shots, for everybody to draw. The
 * arena is not sent: the seed makes it.
 *
 * A guest sends where it is and which way it looks, twenty times a second, and
 * each shot the moment it fires it: where from, which way, and who its own
 * screen saw it meet. The host decides whether that is a hit.
 */
import { ARENA } from './arena'
import { MAX_PLAYERS } from './setup'
import { PITCH_LIMIT, ROUND, SHOT_LIFE, type Claim, type Game, type Player, type Shot } from './rules'

export const SNAPSHOT_TAG = 'hos'
export const MOVE_TAG = 'hos-mv'
export const SHOT_TAG = 'hos-sh'

/** `[id, x cm, z cm, yaw mrad, out cs or -1, by or -1, kills, flags: 1 left]`. */
...
export function encodeSnapshot(game: Game): Record<string, unknown> {
  return {
    ...
    o: game.over ? 1 : 0,
    p: game.players.map(
      (p): WirePlayer => [p.id, cm(p.x), cm(p.z), Math.round(p.yaw * 1000), p.out === null ? -1 : cm(p.out), p.by ?? -1, p.kills, p.left ? 1 : 0],
    ),
    h: game.shots
...
export function decodeSnapshot(message: Record<string, unknown>): Snapshot | null {
  ...
  for (const raw of message.p) {
    ...
    const [id, x, z, yaw, out, by, kills, flags] = raw
    ...
    players.push([id, x, z, yaw, out, by, kills, flags])
  }
  ...
  return { id: message.g as number, seed: message.s as number, elapsed: message.e, over: message.o === 1, players, shots }
}
 ...
 * being eliminated, kills, and leaving. Shots the guest has not seen are added,
 * except its own, which it drew the moment it fired.
 */
export function applySnapshot(game: Game, snap: Snapshot, me: string): Game {
  ...
  for (const [id, x, z, yaw, out, by, kills, flags] of snap.players) {
    ...
    else Object.assign(player, shared, { x: x / 100, z: z / 100, yaw: yaw / 1000 })
    next.push(player)
  }
  game.players = next
...
export function encodeMove(game: number, p: Pick<Player, 'x' | 'z' | 'yaw' | 'pitch'>): Record<string, unknown> {
  return { t: MOVE_TAG, g: game, x: fixed(p.x), z: fixed(p.z), y: fixed(p.yaw), p: fixed(p.pitch) }
}

export function decodeMove(message: Record<string, unknown>): { game: number; x: number; z: number; yaw: number; pitch: number } | null {
  ...
  return { game: message.g as number, x: message.x, z: message.z, yaw: message.y, pitch: message.p }
}

export function encodeShot(game: number, c: Claim): Record<string, unknown> {
  return { t: SHOT_TAG, g: game, x: fixed(c.x), z: fixed(c.z), y: fixed(c.yaw), p: fixed(c.pitch), v: c.victim ?? '' }
}
...
export function decodeShot(message: Record<string, unknown>): ({ game: number } & Claim) | null {
  ...
  return { game: move.game, x: move.x, z: move.z, yaw: move.yaw, pitch: move.pitch, victim: message.v === '' ? null : message.v }
}
