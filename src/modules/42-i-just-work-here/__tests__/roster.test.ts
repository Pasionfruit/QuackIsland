/**
 * Who is dealt into the office while in a lobby.
 */
...
describe('the roster in a lobby', () => {
  ...
  it('is the host and everybody else, with no stand-ins, each at a desk of their own', () => {
    ...
    expect(new Set(game.players.map((p) => p.slot)).size).toBe(2)
