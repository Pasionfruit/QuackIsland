 ...
 * the same rule as the other minigames. Alone it is you and two stand-ins: every
 * turn is watched, so the fewer to watch the better.
 */
...
/** A seed for the order and the outlines, and so an id to tell games apart. */
export function nextSeed(): number {
  dealt += 1
  return hashSeed(CONVENTIONS.worldSeed, `chef-caricature:${getNet().room ?? 'solo'}:${dealt}:${Date.now()}`)
}
