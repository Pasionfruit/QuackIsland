/**
 * Who is dealt into the arena while in a lobby.
 */
...
import { MAX_PLAYERS, SOLO_PLAYERS, newRound, roundRoster } from '../internal/setup'
...
describe('the roster in a lobby', () => {
  ...
  it('is the host and everybody else, with no stand-ins', () => {
    ...
    expect(round.players.find((p) => p.mine)?.id).toBe('p1')
  })

  it('stops at eight', () => {
    lobby.peers = Array.from({ length: 11 }, (_, i) => ({ id: `p${i + 2}`, name: `g${i}`, ping: null }))
    expect(roundRoster()).toHaveLength(MAX_PLAYERS)
  })
})
