/**
 * Putting a game together: who is shopping.
 *
...
export const ME = 'you'
/** You and five stand-ins: a crowd in the aisles. */
...
/** A seed for the lists and the shelves, and so an id to tell games apart. */
export function nextSeed(): number {
  dealt += 1
  return hashSeed(CONVENTIONS.worldSeed, `needs-a-walmart:${getNet().room ?? 'solo'}:${dealt}:${Date.now()}`)
}
...
/** A store with nobody in it yet, for a guest to hold until the host's first snapshot. */
