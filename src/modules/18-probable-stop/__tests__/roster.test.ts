/**
 * Who is dealt into a game while in a lobby - including the lobby of one.
 */
...
import { SOLO_PLAYERS, gameRoster, newGame } from '../internal/setup'
...
describe('the roster in a lobby', () => {
  ...
  it('is the host and then everybody else, with no stand-ins', () => {
    ...
    const game = newGame()
    expect(game.players.map((p) => p.id)).toEqual(['p1', 'p2', 'p10'])
