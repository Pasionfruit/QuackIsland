/**
 * Who is dealt into the kitchen while in a lobby.
 */
...
import { MAX_COOKS, SOLO_COOKS, gameRoster, newGame } from '../internal/setup'
...
describe('the roster in a lobby', () => {
  ...
  it('is you and stand-ins when nobody else has arrived', () => {
    ...
    expect(roster).toHaveLength(SOLO_COOKS)
    expect(roster[0]).toEqual({ id: 'p1', bot: false })
    expect(roster.slice(1).every((r) => r.bot)).toBe(true)
  })

  it('is the host and everybody else, with no stand-ins, and a fresh secret seed each game', () => {
    ...
    expect(newGame().seed).not.toBe(game.seed)
  })

  it('stops at eight', () => {
    lobby.peers = Array.from({ length: 11 }, (_, i) => ({ id: `p${i + 2}`, name: `g${i}`, ping: null }))
    expect(gameRoster()).toHaveLength(MAX_COOKS)
  })
})
