/**
 * Putting a game together: who is on the dock.
 *
...
export const ME = 'you'
/** You and five stand-ins: somebody to fish against. */
...
/** A seed for the bites, and so an id to tell games apart. */
export function nextSeed(): number {
  dealt += 1
  return hashSeed(CONVENTIONS.worldSeed, `milf-fishing:${getNet().room ?? 'solo'}:${dealt}:${Date.now()}`)
}
...
/** A dock with nobody on it yet, for a guest to hold until the host's first snapshot. */
