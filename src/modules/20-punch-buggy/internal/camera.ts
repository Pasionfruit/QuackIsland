 ...
 *
 * High and back from the platform, tilted so you can judge how far a fist is
 * from a body - which is the whole game - while the whole platform, edge to
 * edge, is in view for the whole round. Nobody is ever punched out of sight.
 *
 * Fitted the same way as the other minigames' cameras: slid along its line of
 * sight until the platform touches the edge of the frame, and aimed so the
 * space above and below comes out even.
 */
import { RING } from './rules'

/** The platform, edge to edge, from a little under its top to a head's height above. */
export const BOUNDS = {
  minX: -RING.radius - 0.8,
  maxX: RING.radius + 0.8,
  minZ: -RING.radius - 0.8,
  maxZ: RING.radius + 0.8,
  minY: -1.2,
  maxY: 1.8,
} as const

/** Degrees up from the platform. Steep enough to judge the gap between a fist and a body. */
