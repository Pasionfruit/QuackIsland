/**
 * Who is dealt a lane while in a lobby.
 */
...
import { MAX_RACERS, SOLO_RACERS, newRace, raceRoster } from '../internal/setup'
...
describe('the roster in a lobby', () => {
  ...
  it('is the host and everybody else, with no stand-ins', () => {
    ...
    const race = newRace()
    expect(race.racers.map((r) => r.id)).toEqual(['p1', 'p2'])
    expect(race.racers.some((r) => r.bot)).toBe(false)
    expect(race.racers.filter((r) => r.mine).map((r) => r.id)).toEqual(['p1'])
  })

  it('stops at eight', () => {
    lobby.peers = Array.from({ length: 11 }, (_, i) => ({ id: `p${i + 2}`, name: `g${i}`, ping: null }))
    expect(raceRoster()).toHaveLength(MAX_RACERS)
  })
})
