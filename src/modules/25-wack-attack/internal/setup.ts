...
export const ME = 'you'
export const SOLO_WHACKERS = 4
/** Eight colours. */
export const MAX_WHACKERS = 8
...
export function gameRoster(): { id: string; bot: boolean }[] {
  ...
  if (net.status !== 'joined' || others.length === 0) {
    return [
      { id: mine, bot: false },
      ...Array.from({ length: SOLO_WHACKERS - 1 }, (_, i) => ({ id: `whacker ${i + 2}`, bot: true })),
    ]
  }
  return [mine, ...others].slice(0, MAX_WHACKERS).map((id) => ({ id, bot: false }))
}

let dealt = 0

/**
 * A seed for the moles, and so an id to tell rounds apart. Nothing secret:
 * every browser draws the same moles from it, and knowing when a mole comes up
 * does not walk you over to it.
 */
export function nextSeed(): number {
  dealt += 1
  return hashSeed(CONVENTIONS.worldSeed, `wack-attack:${getNet().room ?? 'solo'}:${dealt}:${Date.now()}`)
}
