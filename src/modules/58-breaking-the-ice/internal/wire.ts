 ...
 * camera-relative movement and yaw, plus **how many times it has jumped and
 * pushed, ever** - running counts, the same idea as every other minigame's
 * click count, so a dropped or repeated message never loses or doubles an
 * action.
 *
 ...
 * the **sparse set of tiles somebody has actually stood on** goes over the
 * wire, and only the ones the shrink has not already claimed on its own,
 * since every client works that part out for itself. The set only ever
...
/** `[tileIndex, crackedAt cs]`. */
export type WireTile = [number, number]
...
/** Every tile somebody has marked that the shrink has not already independently claimed. */
  ...
  for (let layer = 0; layer < LAYERS.length; layer++) {
    ...
    for (let row = 0; row < DIM; row++) {
      for (let col = 0; col < DIM; col++) {
        ...
        if (crackedAt === null) continue
        if (shrunk(l, row, col, round.elapsed)) continue
        out.push([i, cs(crackedAt)])
      }
    }
  }
  return out
...
export function applyTiles(round: Round, sync: TileSync): Round {
  ...
  for (const [index, at] of sync.tiles) round.tiles.crackedAt[index] = at / 100
  return round
}
 ...
 * round: a guest still repeating last round's count of five pushes, as the
 * next round starts, must not throw five pushes into it.
 */
...
export function decodeIntent(message: Record<string, unknown>): { round: number; intent: Intent } | null {
  ...
  if (!isCount(message.j) || !isCount(message.u)) return null
  const length = Math.hypot(message.x, message.z)
  const scale = length > 1 ? 1 / length : 1
  const yaw = Math.atan2(Math.sin(message.w / 1000), Math.cos(message.w / 1000))
  const intent: Intent = { x: message.x * scale, z: message.z * scale, yaw, jumps: message.j, pushes: message.u }
  ...
  for (const [id, x, z, y, yaw, layer, grounded, alive, eliminatedAt] of snap.players) {
    ...
    if (!p) {
      p = {
        ...
        knockX: 0, knockZ: 0, stunUntil: 0, jumps: 0, pushes: 0, pushAt: -Infinity,
