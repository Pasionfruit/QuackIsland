/**
 * Putting a game together: who is on the deck.
 *
...
export const ME = 'you'
/** You and five stand-ins: a crowd to dodge through, and to shove. */
...
/** A seed for the barrage, and so an id to tell games apart. */
export function nextSeed(): number {
  dealt += 1
  return hashSeed(CONVENTIONS.worldSeed, `shanty-matrix:${getNet().room ?? 'solo'}:${dealt}:${Date.now()}`)
}
...
/** A deck with nobody on it yet, for a guest to hold until the host's first snapshot. */
