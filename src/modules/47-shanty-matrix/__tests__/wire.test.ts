...
import { barrageFor } from '../internal/deck'
...
describe('a snapshot', () => {
  it('carries every player, and a guest keeps its own facing', () => {
    ...
    expect(copy.seed).toBe(game.seed)
  ...
  it('carries somebody overboard: out, who shoved them, and how far they have flown', () => {
    const game = host(3)
    const first = barrageFor(game.seed)[0]
    Object.assign(game.players[1], { x: first.cx, z: first.cz })
    game.players[1].shovedBy = 0
    game.players[1].shovedAt = 0
    while (game.players[1].out === null && game.elapsed < 10) {
      game.players[1].shovedAt = game.elapsed
      stepGame(game, 1 / 30)
    }
    for (let i = 0; i < 20; i++) stepGame(game, 1 / 30)
    ...
    expect(copy.players[1].y).toBeCloseTo(game.players[1].y, 1)
  ...
  it('refuses anything malformed, whole', () => {
    ...
    expect(broken((m) => ((m.p as unknown[][])[0][3] = 5000))).toBe(null)
...
describe('what a guest sends', () => {
  it('goes there and back, and nonsense does not', () => {
    const i = { game: 3, mx: 0.707, mz: -0.707, clicks: 4 }
