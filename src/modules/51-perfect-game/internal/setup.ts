/**
 * Putting a game together: who throws.
 *
 * **The people in the lobby are the players**, host first - roster order is
 * colour order - up to eight. Stand-ins fill in only when there
...
export const ME = 'you'
/** You and two stand-ins: every turn is watched, so three turns is plenty. */
export const SOLO_PLAYERS = 3
...
/** A seed for the columns and the order, and so an id to tell games apart. */
export function nextSeed(): number {
  dealt += 1
  return hashSeed(CONVENTIONS.worldSeed, `perfect-game:${getNet().room ?? 'solo'}:${dealt}:${Date.now()}`)
}
...
/** A beach with nobody on it yet, for a guest to hold until the host's first snapshot. */
