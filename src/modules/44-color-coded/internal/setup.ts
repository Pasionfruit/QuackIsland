/**
 * Putting a game together: who is on the panels.
 *
...
export const ME = 'you'
/** You and five stand-ins: a crowd to shove through. */
...
/** A seed for the colours, and so an id to tell games apart. */
export function nextSeed(): number {
  dealt += 1
  return hashSeed(CONVENTIONS.worldSeed, `color-coded:${getNet().room ?? 'solo'}:${dealt}:${Date.now()}`)
}
...
/** An arena with nobody on it yet, for a guest to hold until the host's first snapshot. */
