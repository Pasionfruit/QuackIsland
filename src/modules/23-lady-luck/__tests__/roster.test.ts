/**
 * Who is dealt into the field while in a lobby.
 */
...
import { MAX_HUNTERS, SOLO_HUNTERS, gameRoster, newGame } from '../internal/setup'
...
describe('the roster in a lobby', () => {
  ...
  it('is you and stand-ins when nobody else has arrived', () => {
    ...
    expect(roster).toHaveLength(SOLO_HUNTERS)
    expect(roster[0]).toEqual({ id: 'p1', bot: false })
    expect(roster.slice(1).every((r) => r.bot)).toBe(true)
  })

  it('is the host and everybody else, with no stand-ins, and a fresh field and luck each round', () => {
  ...
  it('stops at eight', () => {
    ...
    expect(gameRoster()).toHaveLength(MAX_HUNTERS)
  })
})
