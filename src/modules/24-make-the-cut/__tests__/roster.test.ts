/**
 * Who is dealt onto the tower while in a lobby.
 */
...
import { MAX_CUTTERS, SOLO_CUTTERS, gameRoster, newGame } from '../internal/setup'
...
describe('the roster in a lobby', () => {
  ...
  it('is you and stand-ins when nobody else has arrived', () => {
    ...
    expect(roster).toHaveLength(SOLO_CUTTERS)
    expect(roster[0]).toEqual({ id: 'p1', bot: false })
    expect(roster.slice(1).every((r) => r.bot)).toBe(true)
  })

  it('is the host and everybody else, with no stand-ins, and fresh secrets each game', () => {
    ...
    expect(game.count).toBe(7)
    const next = newGame()
    expect([next.seed, next.luck]).not.toEqual([game.seed, game.luck])
  })

  it('stops at eight', () => {
    lobby.peers = Array.from({ length: 11 }, (_, i) => ({ id: `p${i + 2}`, name: `g${i}`, ping: null }))
    expect(gameRoster()).toHaveLength(MAX_CUTTERS)
  })
})
