 ...
 * every bomb, the hole, the pin and the end - from its own hands, the
 * stand-ins', and what each guest says its hands are doing, and sends it twenty
 * times a second. Shoving and bombs are physics between bodies, so one screen
 * owns all of it.
 *
 ...
 * screen and is eased towards where the host has it; a shove or a blast that
 * lands on it comes from the host, and wins. Scanning never goes on the wire:
 * it is the screen's own.
 *
...
import { BODY, clock, inRoom, judgeEnd, leave, move, push, steer, tick, type Game } from './rules'
...
export function useRoomNet(): RoomNet {
  ...
  const advance = (game: Game, dt: number, hands: Hands | null, paused: boolean) => {
    ...
    if (mine && hands) {
      ...
      if (inRoom(mine) && !game.over && Math.hypot(hands.mx, hands.mz) > 1e-6) mine.yaw = Math.atan2(-hands.mx, -hands.mz)
      const host = own.current.host
      if (host && inRoom(mine) && !game.over) {
