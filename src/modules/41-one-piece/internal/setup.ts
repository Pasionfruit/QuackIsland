...
export function gameRoster(): { id: string; bot: boolean }[] {
  ...
  if (net.status !== 'joined' || others.length === 0) {
    return [
      { id: mine, bot: false },
      ...Array.from({ length: SOLO_PLAYERS - 1 }, (_, i) => ({ id: `puzzler ${i + 2}`, bot: true })),
    ]
  }
...
/** A seed for the table - where the pieces start and how they are turned - and so an id to tell games apart. */
export function nextSeed(): number {
  dealt += 1
  return hashSeed(CONVENTIONS.worldSeed, `one-piece:${getNet().room ?? 'solo'}:${dealt}:${Date.now()}`)
}
