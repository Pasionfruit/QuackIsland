 ...
 * colour order and staircase order - up to eight. Stand-ins fill in only when
 * there is nobody else, the same rule as the other minigames.
 */
...
export const ME = 'you'
export const SOLO_STEPPERS = 4
/** Eight colours, eight staircases. */
export const MAX_STEPPERS = 8
...
export function gameRoster(): { id: string; bot: boolean }[] {
  ...
  if (net.status !== 'joined' || others.length === 0) {
    return [
      { id: mine, bot: false },
      ...Array.from({ length: SOLO_STEPPERS - 1 }, (_, i) => ({ id: `stepper ${i + 2}`, bot: true })),
    ]
  }
  return [mine, ...others].slice(0, MAX_STEPPERS).map((id) => ({ id, bot: false }))
}

let dealt = 0

/** A seed for picks made for players who did not pick, and so an id to tell games apart. */
export function nextSeed(): number {
  dealt += 1
  return hashSeed(CONVENTIONS.worldSeed, `synchronize-steps:${getNet().room ?? 'solo'}:${dealt}:${Date.now()}`)
}
