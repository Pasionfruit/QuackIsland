/**
 * Putting a game together: who is in the maze.
 *
...
export const ME = 'you'
export const SOLO_TORCHES = 4
/** Eight colours, eight torches. */
...
export function gameRoster(): { id: string; bot: boolean }[] {
  ...
  if (net.status !== 'joined' || others.length === 0) {
    return [
      { id: mine, bot: false },
      ...Array.from({ length: SOLO_TORCHES - 1 }, (_, i) => ({ id: `helper ${i + 2}`, bot: true })),
    ]
  }
  return [mine, ...others].slice(0, MAX_TORCHES).map((id) => ({ id, bot: false }))
}

let dealt = 0

/** A seed for the maze, and so an id to tell games apart. */
export function nextSeed(): number {
  dealt += 1
  return hashSeed(CONVENTIONS.worldSeed, `helping-dad:${getNet().room ?? 'solo'}:${dealt}:${Date.now()}`)
}
