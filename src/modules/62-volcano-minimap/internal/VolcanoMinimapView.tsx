...
function DiceRoll({ dice, rollKey }: { dice: readonly BoardDieRoll[]; rollKey: string }) {
  return <div aria-label={`Latest roll: ${dice.map((die) => die.value).join(', ')}`} style={styles.diceTray}>
    {dice.map((die, index) => <Die die={die} index={index} key={`${rollKey}:${index}:${die.value}`} />)}
  </div>
}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  diceTray: { display: 'flex', justifyContent: 'center', gap: 10, marginTop: 10, minHeight: 42, perspective: 180 },
