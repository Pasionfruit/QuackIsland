/**
 * Putting a game together: who is playing.
 *
...
export function gameRoster(): { id: string; bot: boolean }[] {
  ...
  if (net.status !== 'joined' || others.length === 0) {
    return [
      { id: mine, bot: false },
      ...Array.from({ length: SOLO_PLAYERS - 1 }, (_, i) => ({ id: `clock ${i + 2}`, bot: true })),
    ]
  }
...
/** A seed for the three targets, and so an id to tell games apart. Not a secret: the targets are on everybody's screen. */
export function nextSeed(): number {
  dealt += 1
  return hashSeed(CONVENTIONS.worldSeed, `ill-just-wait:${getNet().room ?? 'solo'}:${dealt}:${Date.now()}`)
}
