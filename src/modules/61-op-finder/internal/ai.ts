 ...
 * sometimes getting it wrong once first and paying the same lockout a person
 * would. Seeded per bot per stage, so the same round plays out the same way
 * every time it is replayed with the same seed - and a slower or faster
 * machine never changes who wins.
 */
