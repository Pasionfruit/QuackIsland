/**
 * Who is dealt onto the stage while in a lobby.
 */
...
import { MAX_TIMERS, SOLO_TIMERS, gameRoster, newGame } from '../internal/setup'
...
describe('the roster in a lobby', () => {
  ...
  it('is you and stand-ins when nobody else has arrived', () => {
    ...
    expect(roster).toHaveLength(SOLO_TIMERS)
  ...
  it('stops at eight', () => {
    ...
    expect(gameRoster()).toHaveLength(MAX_TIMERS)
  })
})
